...
#   - redis: --requirepass 값·"값", redis.conf requirepass 값, JSON 배열 "--requirepass", "값", redis-cli ... -a 값
...
MASK_SED=(
    ...
    -e 's/((^|[^A-Za-z0-9_-])(--)?requirepass[[:space:]]+)[^[:space:]"\x27,]+/\1***/Ig'
    -e 's/(redis-cli([[:space:]]+[^[:space:]]+)*[[:space:]]+-a[[:space:]]+)("([^"\\]|\\.)*"|[^[:space:]]+)/\1***/Ig'
    ...
    -e 's/((^|[^A-Za-z0-9_])[A-Za-z0-9_]*(PASSWORD|PASSWD|PWD|SECRET|TOKEN|_KEY|BASIC_AUTH)[A-Za-z0-9_]*[[:space:]]*[=:][[:space:]]*)[^[:space:]"\x27]+/\1***/Ig'
    -e 's#(://[^:/@[:space:]]*:)[^@[:space:]]+@#\1***@#g'
)
