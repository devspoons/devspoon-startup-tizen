...
if [[ "$yesno" == "y" ]]; then
    # harbor.yml 의 인증서 경로는 <ssl path>/letsencrypt/live/<domain>/ — ssl 폴더의 "내용"을 ssl path 아래로 복사한다
    mkdir -p "$sslpath" && cp -rf ssl/. "$sslpath/" || { echo "ssl 복사 실패: $sslpath (쓰기 권한 확인 — 기본 /etc 는 root 필요)"; exit 1; }
    echo  "ssl key copied to path: $sslpath"
fi
