#!/usr/bin/env bash
# =============================================================================
# Generate per-domain HTTPS nginx conf from sample_nginx_proxy_https.conf
# (proxy-only)
#
# Usage (interactive):
#   ./nginx_proxy_https_conf.sh
#
# Usage (non-interactive):
#   ./nginx_proxy_https_conf.sh --port 80 --domain example.com \
#                               --proxyurl backend --proxyport 8000 \
#                               --certbot-root /www/certbot \
#                               --filename example.com
#
# Output: ./conf.d/<filename>_proxy_https_ng.conf
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SAMPLE="${SCRIPT_DIR}/sample_nginx_proxy_https.conf"
OUT_DIR="${SCRIPT_DIR}/conf.d"
SUFFIX="_proxy_https_ng"
